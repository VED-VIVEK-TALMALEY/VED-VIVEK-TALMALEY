...
# Edit this list with your own details!
ROWS = [
    ("host",),
    ("kv", "Now", "Software Engineer @ YourCompany"),
    ("kv", "Prev", "Founding Engineer @ FormerStartup"),
    ("kv", "Also", "SDE + Open Source Contributor"),
    ("kv", "Edu", "B.Tech Computer Science"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "React, Next.js, TypeScript"),
    ("kv", "Backend", "Node, Python, Go, SQL"),
    ("kv", "Tools", "Docker, AWS, Git, VS Code"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Building cool profile animations"),
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">yourusername@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">yourusername</tspan><tspan fill="{MUTED}">@</tspan>'
